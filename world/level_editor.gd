...
# called when clicking any tile or entity without an entity editor
...
# called when clicking on redo button or CTRL+SHIFT+Z
func redo():
	undo_redo.redo()
# called when clicking on undo button or CTRL+Z
func undo():
	undo_redo.undo()
...
func apply(pos: Vector2) -> void:
	if modes_group.get_pressed_button().name == "add":
		...
		add_entity(pos, selected_object, properties)
	elif modes_group.get_pressed_button().name == "erase":
		erase(pos)
	elif modes_group.get_pressed_button().name == "replace":
		if selected_object_type == Objects.ObjectType.TILE:
			replace_tile(pos, selected_object)
		elif selected_object_type == Objects.ObjectType.ENTITY:
			var properties : Array 
			if( entity_editor):
				properties  = entity_editor.call("get_tile_properties")
			replace_entity(pos, selected_object, properties)


func add_entity(pos: Vector2, selected_object : int, properties : Array) -> void:
	undo_redo.create_action("add_entity")
...
func replace_entity(pos: Vector2, selected_object: int, properties: Array) -> void:
	undo_redo.create_action("replace_entity")
...
func replace_tile(pos: Vector2, selected_object: int) -> void:
	undo_redo.create_action("replace_tile")
